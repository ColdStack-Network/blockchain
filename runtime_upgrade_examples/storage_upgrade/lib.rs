...
pub mod pallet {
  ...
  */
  ...
  impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
      ...
      // TODO check for `StorageVersion` to decide if upgrade was not already
      // performed

      for (address, seed_address) in GatewayNodeSeeds::<T>::drain() {
        Gateways::<T>::insert(address.clone(), Gateway {
          address: address,
          seed_address: seed_address,
        })
      }

      // TODO if upgrade was performed then save new `StorageVersion`
  ...
  impl<T: Config> Pallet<T> {
    ...
    pub fn test(origin: OriginFor<T>, 
      param: u128,
    ) -> DispatchResultWithPostInfo {
      let sender = ensure_signed(origin)?;
      Self::deposit_event(Event::Test(param));
      Ok(().into())
    }
